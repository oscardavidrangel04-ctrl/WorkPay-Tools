...
function calculate(){
 ...
 if(id==='overtime'||id==='weeklyPay'){
   ...
   const rate=Math.max(0,num('rate')),hours=Math.max(0,num('hours')),weeks=Math.min(53,Math.max(0,num('weeks',52)));
   const includeOt=!!document.getElementById('includeOvertime')?.checked,otThreshold=Math.max(0,num('hourlyOtThreshold',40)),otMult=Math.max(1,num('hourlyOtMultiplier',1.5));
   const regularHours=includeOt?Math.min(hours,otThreshold):hours,otHours=includeOt?Math.max(0,hours-otThreshold):0;
   const regularPay=regularHours*rate,otPay=otHours*rate*otMult,weekly=regularPay+otPay,annual=weekly*weeks;
   const rows=[['Hourly rate',money(rate)+'/hr'],['Paid hours / week',hours.toFixed(2)],['Paid weeks / year',weeks.toFixed(0)]];
   if(includeOt)rows.push(['Regular hours',regularHours.toFixed(2)],['Overtime hours',otHours.toFixed(2)],['Overtime rate',money(rate*otMult)+'/hr']);
   rows.push(['Weekly gross',money(weekly)],['Biweekly average',money(annual/26)],['Semimonthly average',money(annual/24)],['Monthly average',money(annual/12)],['Annual gross pay',money(annual)]);
   setResult(`${money(annual)} / year`,rows);
